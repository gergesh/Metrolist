...
class Migration36To37 : AutoMigrationSpec {
    override fun onPostMigrate(db: SupportSQLiteDatabase) {
        var hasContextPlaylistId = false
        var hasContextTitle = false
        db.query("PRAGMA table_info('event')").use { cursor ->
            val nameIndex = cursor.getColumnIndex("name")
            while (cursor.moveToNext()) {
                val colName = if (nameIndex >= 0) cursor.getString(nameIndex) else null
                when (colName) {
                    "contextPlaylistId" -> hasContextPlaylistId = true
                    "contextTitle" -> hasContextTitle = true
                }
            }
        }
        if (!hasContextPlaylistId) {
            db.execSQL("ALTER TABLE event ADD COLUMN contextPlaylistId TEXT DEFAULT NULL")
        }
        if (!hasContextTitle) {
            db.execSQL("ALTER TABLE event ADD COLUMN contextTitle TEXT DEFAULT NULL")
        }
    }
}
